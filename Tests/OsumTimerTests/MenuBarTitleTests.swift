...
final class MenuBarTitleTests: XCTestCase {
    ...
    /// In percent mode both numbers move: "0% (25:00)" through
    /// "100% (0:00)" all measure the same.
    func testEveryPercentIsOneWidth() {
        let widest = "100% (\(Parser.clock(for: 1500)))"
        for remaining in stride(from: 1500.0, through: 0, by: -15) {
            let percent = Int((1 - remaining / 1500) * 100)
            assertSameWidth("\(percent)% (\(Parser.clock(for: remaining)))", widest)
        }
    }
